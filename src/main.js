...
function renderActions(state) {
    ...
    switch (state.phase) {
        case "pick-action":
            container.append(
                btn("FIGHT", selectFight),
                btn("SWITCH", selectSwitch),
            );
            break;
        ...
        case "pick-move": {
            ...
            if (!haspp) {
                container.append(
                    moveBtn(allMoves.struggle, () =>
                        playerFight(allMoves.struggle),
                    ),
                );
            } else {
        ...
        case "faint-switch": {
            ...
            avail.forEach((p, i) => {
                const pct = Math.round((p.currentHp / p.maxHp) * 100);
                container.append(
                    btn(`${p.name}  ${pct}%`, () => playerFaintSwitch(i)),
                );
            });
            break;
        }
        ...
        if (!firstLoad) {
            ...
            if (playerSwitched) {
                ...
                if (player) loadSprite("player", player.name, false);
                    ...
                    loadSprite("player", lastPlayer.name, true);
            }
        }
...
function updateSelectionState() {
    ...
    document.querySelectorAll(".poke-card").forEach((card) => {
        ...
        card.classList.toggle(
            "disabled",
            !isSelected && count >= MAX_TEAM_SIZE,
        );
